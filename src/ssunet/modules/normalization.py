...
from ..constants import LOGGER  # Assuming LOGGER is in constants
...
def get_norm_layer(norm_type: NormTypeOptions, num_features: int, **kwargs: dict) -> nn.Module:
    ...
    if norm_type_lower == "group":
        ...
        # Remove num_groups from kwargs to avoid conflict
        group_kwargs = {k: v for k, v in kwargs.items() if k != "num_groups"}
        return nn.GroupNorm(num_groups, num_features, **group_kwargs)
    ...
    # For unknown normalization types, return Identity as a fallback
    LOGGER.warning(f"Unknown normalization type: {norm_type}. Returning nn.Identity().")
    return nn.Identity()
