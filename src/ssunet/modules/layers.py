...
def pool(
    ...
    if down_mode == "conv":
        # Use nn.Conv3d directly with padding=0 to match MaxPool/AvgPool output shape
        return nn.Conv3d(
            in_channels,
            out_channels,
            kernel_size=kernel_size_pool,
            stride=stride_pool,
            padding=0,  # Ensures output_dim = input_dim / 2
            bias=True,
        )
...
# --- Merge Operation ---
def merge(
    input_a: torch.Tensor, input_b: torch.Tensor | None, merge_mode: MergeModeOptions = "concat"
) -> torch.Tensor:
